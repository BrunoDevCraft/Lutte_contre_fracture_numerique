...
// Chaque section représente une diapositive du cours avec un titre, une icône et un contenu HTML.
        const courseSections = [
            ...
            {icon:"😊",title:"Alors, rassuré(e) ?",
                content:`<div class="card" style="background:linear-gradient(135deg,#e3f2fd 0%,#bbdefb 100%);border:3px solid #2196f3;padding:35px">
                    <div style="text-align:center;margin-bottom:30px">
                    <div style="font-size:6em;margin-bottom:20px">😰 ➜ 😊</div>
                    <h2 style="font-size:2.2em;color:#1565c0;margin-bottom:20px">Récapitulons !</h2>
                    </div><div style="background:white;padding:35px;border-radius:12px;margin-bottom:30px;box-shadow:0 6px 20px rgba(0,0,0,0.1)">
                    <h3 style="color:#667eea;margin-bottom:30px;text-align:center;font-size:1.6em">📋 Ce que vous avez appris :</h3>
                    <div style="display:grid;gap:20px">
                    <div style="background:#e8f5e9;padding:20px;border-radius:10px;border-left:5px solid #4caf50">
                    <p style="font-size:1.1em;line-height:1.7">✅ L'ordinateur n'est <strong>pas un génie</strong>, c'est un assistant obéissant</p></div>
                    <div style="background:#fff3e0;padding:20px;border-radius:10px;border-left:5px solid #ff9800">
                    <p style="font-size:1.1em;line-height:1.7">✅ Il ne vous <strong>juge pas</strong> et vous pouvez faire des erreurs</p>
                    </div><div style="background:#f3e5f5;padding:20px;border-radius:10px;border-left:5px solid #9c27b0">
                    <p style="font-size:1.1em;line-height:1.7">✅ Presque tout peut être <strong>annulé</strong> avec CTRL+Z</p>
                    </div>
                    <div style="background:#e3f2fd;padding:20px;border-radius:10px;border-left:5px solid #2196f3">
                    <p style="font-size:1.1em;line-height:1.7">✅ C'est comme une <strong>télécommande</strong> : vous êtes aux commandes</p>
                    </div>
                    </div>
                    </div>
                    <div style="background:linear-gradient(135deg,#f3e5f5 0%,#e1bee7 100%);padding:35px;border-radius:15px;text-align:center">
                    <h3 style="font-size:2em;color:#7b1fa2;margin-bottom:20px">🎉 Prêt(e) pour la suite ?</h3>
                    <p style="font-size:1.3em;line-height:1.8;color:#333;margin-bottom:25px">Maintenant que vous êtes rassuré(e), découvrons ensemble comment fonctionne concrètement un ordinateur avec des comparaisons simples !</p>
                    <div style="background:white;padding:25px;border-radius:12px;display:inline-block;margin-top:20px">
                    <p style="font-size:1.2em;color:#667eea;font-weight:bold;margin:0">👉 Continuons l'aventure !</p>
                    </div>
                    </div>
                    </div>
                    `},
                    ...
                    <div class="card">
                        ...
                        <div class="grid">
                            <div class="grid-item" style="border-color: #9c27b0;">
                                ...
                                <p class="comparison-detail" style="text-align: center;"> C'est à dire: </p>
                            ...
                            <div class="grid-item" style="border-color: #ff9800;">
                                ...
                                <p class="comparison-detail">C'est à dire: </p>
                                <p class="comparison-detail">Photos, textes, vidéos, musiques...
                                </p>
...
function showResults() {
    ...
    // Mettre à jour les éléments de la page des résultats
...
// Fonctions pour les boutons de la page de résultats
...
function generateDiploma() {
    ...
    if (!userName) {
        alert('Veuillez entrer votre nom pour générer le diplôme.');
        return;
    }
    
    // Récupérer les résultats du localStorage
    const score = localStorage.getItem('quizScore') || quizState.score;
    const total = localStorage.getItem('quizTotal') || quizQuestions.length;
    
    // Mettre à jour la page du diplôme
    document.getElementById('diploma-name').textContent = userName;
    document.getElementById('diploma-score').textContent = `${score}/${total}`;
    ...
    // Afficher la page du diplôme
    showPage('diplome');
}

// Fonction pour imprimer le diplôme
function printDiploma() {
    // Sauvegarder l'état actuel de la page
    const originalContent = document.body.innerHTML;
    const diplomePage = document.getElementById('page-diplome');
    
    if (!diplomePage) {
        alert('Erreur : page du diplôme introuvable');
        return;
    }
    
    // Créer une copie du diplôme pour l'impression
    const printContent = diplomePage.cloneNode(true);
    
    // Masquer les boutons dans la copie
    ...
    setTimeout(() => {
        ...
        // Réattacher les événements en rechargeant les scripts
        showPage('diplome');
    }, 100);
}