...
function renderGameLevels() {
    ...
    // Filtrer pour n'afficher que les 8 premiers niveaux
    const levelsToDisplay = gameLevels.slice(0, 8);
 ...
 */
function toggleTask(taskId, points, taskElement) {
    if (!gameState.completedTasks.includes(taskId)) {
        gameState.completedTasks.push(taskId);
        gameState.totalPoints += points;
        taskElement.classList.add('completed');
        taskElement.querySelector('.task-dot').textContent = '✓';
    } else {
    ...
    saveGameState();
    renderGameLevels();
    document.getElementById('game-points').textContent = gameState.totalPoints;
}
